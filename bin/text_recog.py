...
def word_seg(inputData):
    ...
    x= [tok for tok in listOfTokens if (tok not in stopWords)] # 注意python3 写成一行的if 语句需要用括号
    x=list(set(x)) # 使用词集
    '''
    词集和词袋的区别：
    词集里面只考虑每个词是否出现
